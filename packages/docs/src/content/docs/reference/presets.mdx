...
- **Absolute Difference Less Than**: Check if `abs(value - otherNumber) < threshold`
- **Absolute Difference Less Than Or Equal**: Check if `abs(value - otherNumber) <= threshold`

### Date Filters

- **Before**: Check if a date is before another date
- **After**: Check if a date is after another date
- **Between Days Before**: Check if a date is inclusively N to M days before another date
- **Between Days Before Exclusive**: Check if a date is exclusively N to M days before another date
...
Filter arguments can use literal values, compatible field references, or supported expressions where the filter signature allows them. For example, the absolute difference and days-before filters can compare two fields from the same row.
